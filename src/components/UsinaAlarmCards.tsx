  ...
  useEffect(() => {
    const fetchAlarms = async () => {
      try {
        ...
        setActiveAlarms((atuaisRes.data.alarmes_atuais || []).map(formatAlarm));
        setResolvedAlarms((historicoRes.data.alarmes_historicos || []).map(formatAlarm));
      } catch (error) {
          ...
          <Tabs defaultValue="active" className="w-full">
                    ...
                    <div className="flex justify-end pt-2">
                      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                        ...
                        <DialogContent className="max-h-[80vh] overflow-y-auto">
                          <h2 className="text-lg font-semibold mb-4">Todos os Alarmes Ativos</h2>
            ...
            <TabsContent value="history" className="mt-4 space-y-3">
              {resolvedAlarms.length > 0 ? (
                <>
                  {resolvedAlarms.slice(0, 3).map((alarm) => (
                    <AlarmItem key={alarm.id} alarm={alarm} />
                  ))}
                  {resolvedAlarms.length > 3 && (
                    <div className="flex justify-end pt-2">
                      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                        <DialogTrigger asChild>
                          <Button variant="outline" size="sm">
                        ...
                        <DialogContent className="max-h-[80vh] overflow-y-auto">
                          <h2 className="text-lg font-semibold mb-4">Histórico Completo de Alarmes</h2>
                          <div className="space-y-3">
